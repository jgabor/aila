...
func (runner *inputRunner) proposeCapability(request capability.Request) tui.TranscriptTurn {
	...
	case capability.NameVision:
		turn.Vision = visionView(runner.model.LastCapability, request.Phase, visionArtifactPersistence{})
		if turn.Vision != nil {
	...
	case capability.NameResearch:
		turn.Research = researchView(runner.model.LastCapability, request.Phase)
		if turn.Research != nil {
			turn.StatusDetail = "research capability status"
		}
	case capability.NameProfile:
		turn.Profile = profileView(runner.model.LastCapability, request.Phase, profileArtifactPersistence{})
		if turn.Profile != nil {
	...
	case capability.NameBuild:
		turn.Build = buildView(runner.model.LastCapability, request.Phase)
		if turn.Build != nil {
	...
	case capability.NameDocument:
		turn.Document = documentView(runner.model.LastCapability, request.Phase, documentArtifactPersistence{})
		if turn.Document != nil {
			turn.StatusDetail = "document capability status"
		}
	case capability.NameDesign:
		turn.Design = designView(runner.model.LastCapability, request.Phase, designArtifactPersistence{})
		if turn.Design != nil {
	...
	case capability.NameAudit:
		...
		if turn.Audit != nil {
			turn.StatusDetail = "audit capability status"
		}
	}
	return turn
}
