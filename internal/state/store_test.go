...
func TestResolveArtifactWriteAllowsOwningWriter(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: ArtifactDocumentation, owner: OwnerApp},
	} {
		t.Run(string(tc.name), func(t *testing.T) {
			t.Parallel()
...
func TestWriteArtifactStoresOwnedContentWithAtomicBoundary(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: ArtifactProfile, owner: OwnerApp, content: "# Profile\n\nSubject: Aila decision profile\n"},
	} {
		tc := tc
		t.Run(string(tc.name), func(t *testing.T) {
			t.Parallel()
...
func TestWriteAppOwnedArtifactsRejectStateOwnerWithoutFinalMutation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: ArtifactProfile, content: "# Existing profile\n", rejected: "bad profile\n"},
	} {
		t.Run(string(tc.name), func(t *testing.T) {
			artifact := mustResolveArtifact(t, store.Resolver(), tc.name)
